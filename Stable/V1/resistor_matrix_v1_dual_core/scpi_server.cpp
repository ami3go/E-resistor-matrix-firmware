...
void scpiPrintHelp(WiFiClient& client) {
  ...
  client.println("DIAG:LATCHPINS?                     - show channel-to-GPIO latch map");
  client.println("DIAG:BRANCH CH<n>,<bit>             - safe exclusive single-branch test");
...
/**
 * @brief Print the configured channel-to-GPIO latch-pin map.
 *
 * This is a commanded/configuration map, not electrical feedback. Use it to
 * compare firmware assumptions against the PCB schematic and logic-analyzer
 * measurements. In the current board configuration CH6 is latched by GP10.
 */
static void scpiPrintLatchPins(WiFiClient& client) {
  for (uint8_t ch = 0; ch < CHANNEL_COUNT; ch++) {
    if (ch > 0) {
      client.print(",");
    }
    ...
    client.print(ch + 1);
    client.print("=GP");
    client.print(SR_LATCH_PINS[ch]);
  }
  client.println();
}

/**
 * @brief Parse a diagnostic branch selector: "CH6,0", "6,0", or "CH6 BIT0".
 * @param args Argument text following DIAG:BRANCH.
 * @param channelIndex Parsed zero-based channel index.
 * @param bitIndex Parsed zero-based branch bit index.
 * @return true if both channel and bit were valid.
 */
static bool parseDiagnosticBranchArgs(String args, uint8_t& channelIndex, uint8_t& bitIndex) {
  args.trim();
  args.replace("=", " ");
  args.replace(",", " ");
  args.replace(";", " ");
  ...
  String upper = args;
  upper.toUpperCase();
  upper.replace("CHANNEL", "CH");
  upper.replace("CHAN", "CH");
  upper.replace("BIT", " ");
  upper.replace("Q", " ");
  upper.trim();
  ...
  for (int pos = 0; pos < int(upper.length()) && valueCount < 2; ) {
    while (pos < int(upper.length()) && !isdigit(upper[pos])) {
      pos++;
    }
    if (pos >= int(upper.length())) {
      break;
    }

    int value = 0;
    while (pos < int(upper.length()) && isdigit(upper[pos])) {
      value = value * 10 + (upper[pos] - '0');
      pos++;
    }
  ...
  const int ch = values[0];
  const int bit = values[1];
  if (ch < 1 || ch > int(CHANNEL_COUNT) || bit < 0 || bit >= int(BIT_COUNT)) {
    return false;
  }
...
/**
 * @brief Execute a safe exclusive single-branch diagnostic command.
 *
 * The command first builds a complete eight-channel mask image with every
 * channel OFF except the selected branch, then sends it through the normal
 * Core-1 safe apply path. This removes ambiguity from calibration: if DMM still
 * reads open on CH6 after DIAG:BRANCH CH6,0, the firmware has commanded exactly
 * one branch and the remaining fault is latch-map or hardware related.
 */
static bool tryHandleDiagnosticCommand(String cmd, WiFiClient& client) {
  String upper = cmd;
  upper.toUpperCase();
  upper.trim();

  if (upper == "DIAG:LATCHPINS?" || upper == "DIAG:LATCH:PINS?" || upper == "DIAG:PINMAP?") {
    scpiPrintLatchPins(client);
    return true;
  }

  const char* branchPrefixes[] = {
    "DIAG:BRANCH",
    "DIAG:CHAN:BRANCH",
    "DIAG:CHANNEL:BRANCH",
    "DIAG:EXCLUSIVE",
  ...
  for (size_t i = 0; i < sizeof(branchPrefixes) / sizeof(branchPrefixes[0]); i++) {
    const String prefix(branchPrefixes[i]);
    ...
    uint8_t ch = 0;
    uint8_t bit = 0;
    if (!parseDiagnosticBranchArgs(args, ch, bit)) {
      setLastError("-128,\"Bad DIAG:BRANCH args\"");
      client.println("ERR,-128,\"Use DIAG:BRANCH CH<n>,<bit 0..15>\"");
      return true;
    }
    ...
    snprintf(
      msg,
      sizeof(msg),
      "DIAG exclusive CH%u bit%u mask=0x%04X latch=GP%u",
      unsigned(ch + 1),
      unsigned(bit),
      unsigned(masks[ch]),
      unsigned(SR_LATCH_PINS[ch])
    );
    setStatus(msg);
    appendLogEvent(msg);
    clearLastError();
    ...
    client.print(bit);
    client.print(",MASK=");
    client.print(hex16(masks[ch]));
    client.print(",LATCH=GP");
    client.println(SR_LATCH_PINS[ch]);
    return true;
  }
...
void processScpiLine(const char* rawLine, WiFiClient& client) {
  ...
  if (upper == "*IDN?") {
    client.print("OpenBench,RP2040-Resistor-Matrix,");
  ...
  if (tryHandleDiagnosticCommand(cmd, client)) {
    return;
  }
