...
inline constexpr uint16_t SR_CLOCK_HALF_PERIOD_US = 25;   // intentionally slow: avoids ringing/double-clock on long PCB/wires
inline constexpr uint16_t SR_DATA_SETUP_US        = 20;
inline constexpr uint16_t SR_PRE_LATCH_SETTLE_US  = 100;  // protects bit 0 from latch/clock crosstalk
inline constexpr uint16_t SR_LATCH_SETUP_US       = 25;
inline constexpr uint16_t SR_LATCH_PULSE_US       = 100;
inline constexpr uint16_t SR_POST_LATCH_SETTLE_US = 100;
inline constexpr uint16_t SR_CLEAR_PULSE_US       = 100;
inline constexpr uint16_t BREAK_BEFORE_MAKE_MS    = 3;
inline constexpr uint8_t SR_FINAL_WRITE_REPEATS   = 2;    // repeat final write to recover from a marginal missed latch
inline constexpr bool SR_SHIFT_BIT0_FIRST         = true; // required mapping: mask bit 0 -> Q16
inline constexpr bool SR_VERBOSE_LOG              = false;
...
extern volatile uint16_t channelMask[CHANNEL_COUNT];
extern volatile uint32_t applyCounter[CHANNEL_COUNT];

// High-level safety state. Pinout is unchanged.
// These flags are touched by both RP2040 cores, so keep them volatile.
extern volatile bool shiftRegistersReady;
extern volatile bool outputsKnownSafe;
extern volatile bool fatalSafeStateActive;
